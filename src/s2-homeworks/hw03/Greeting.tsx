import React, { ChangeEvent, KeyboardEvent } from 'react'
import s from './Greeting.module.css'
import { UserType } from './HW3'
import {v1} from 'uuid'

type GreetingPropsType = {
    users: Array<UserType> 
    name: string // need to fix any
    setNameCallback: (name: string) => void // need to fix any
    addUser: ()=>void // need to fix any
    onBlur: ()=>void // need to fix any
    onEnter: (key: string) => void // need to fix any
    error: string // need to fix any
    totalUsers: number // need to fix any
    lastUserName?: string | boolean // need to fix any
}

// презентационная компонента (для верстальщика)
const Greeting: React.FC<GreetingPropsType> = (
    {
        users,
        name,
        setNameCallback,
        addUser,
        onEnter,
        onBlur,
        error,
        totalUsers,
        lastUserName,
    } // деструктуризация пропсов
) => {
    const inputClass = s.errorInput // need to fix with (?:)

    return (
        <div id={'hw3-form'} className={s.greetingForm}>
            <div className={s.text}>
                {'Людей добавили: '}
                <span id={'hw3-users-total'}>
                    {totalUsers}
                </span>
            </div>

            <div className={s.inputAndButtonContainer}>
                <div>
                    <input
                        id={'hw3-input'}
                        value={name}
                        onChange={(e)=>setNameCallback(e.currentTarget.value)}
                        className={error?s.errorInput:s.input}
                        onKeyDown={(e)=>onEnter(e.key)}
                        onBlur={onBlur}    /*  что здесь писать ????? */
                    />

                    <button
                        id={'hw3-button'}
                        onClick={addUser}
                        className={s.button}
                        disabled={!name.trim()}
                    >
                        Add
                    </button>

                    <div id={'hw3-error'} className={s.error}>
                        {error}
                    </div>
                </div>


            </div>

            {/* TODO:  здесь нужен промаппить массив юзеров , вытаскиваю оттуда имена */}
           {/*  {lastUserName && (
                <div className={s.greeting}>
                    Привет <span id={'hw3-last-user'}>{lastUserName}</span>!
                </div>
            )} */}


            {users.map(el=>{
                return(
                    <div key={v1()} className={s.greeting}>
                    Привет <span id={'hw3-last-user'}>{el.name}</span>!
                </div>
                )
            })}



        </div>
    )
}

export default Greeting
